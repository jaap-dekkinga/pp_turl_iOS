...
protocol DMSwipeCardDelegate: AnyObject {
...
class DMSwipeCard: UIView {
	...
	func configureOverlays() {
		configureOverlay(overlay: leftOverlay)
		configureOverlay(overlay: rightOverlay)
	}
	...
	@objc func dragEvent(gesture: UIPanGestureRecognizer) {
		...
		switch gesture.state {
			...
			case .changed:
				...
				let scale = min(1 - abs(rStrength) / self.scaleStrength, self.scaleMax)
	...
	private func updateOverlay(_ distance: CGFloat) {
		...
		activeOverlay?.alpha = min(abs(distance)/100, 1.0)
	}
